...
The input will always be valid (numbers will be an array of length 
    2 or greater, and all of the items will be numbers; target will 
...
function twoSum(numbers, target) {
    ...
    for(let i = 0; i < numbers.length; i++){
      if(dictionary[`${numbers[i]}`] !== undefined){
        output[0] = dictionary[`${numbers[i]}`]
        output[1] = i
      }
      else{
        dictionary[`${target - numbers[i]}`] = i
      }
    }
      return output;
  }
